...
# original can be found here: <https://github.com/hyperledger/fabric-samples/blob/release-1.4/first-network/scripts/utils.sh>
...
setOrdererGlobals() {
  ...
  CORE_PEER_TLS_ROOTCERT_FILE=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/ordererOrganizations/tfc.com/orderers/orderer.tfc.com/msp/tlscacerts/tlsca.tfc.com-cert.pem
  CORE_PEER_MSPCONFIGPATH=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/ordererOrganizations/tfc.com/users/Admin@tfc.com/msp
}
...
updateAnchorPeers() {
  ...
  if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
    set -x
    peer channel update -o orderer.tfc.com:7050 -c $CHANNEL_NAME -f ./channel-artifacts/Player${PLAYER}anchors.tx >&log.txt
    res=$?
    set +x
  else
    set -x
    peer channel update -o orderer.tfc.com:7050 -c $CHANNEL_NAME -f ./channel-artifacts/Player${PLAYER}anchors.tx --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA >&log.txt
    res=$?
    set +x
  fi
  cat log.txt
...
instantiateChaincode() {
  ...
  if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
    set -x
    peer chaincode instantiate -o orderer.tfc.com:7050 -C $CHANNEL_NAME -n ${CCNAME} -l ${LANGUAGE} -v ${VERSION} -c "${INSTARGS}" -P ${PERMISSION} >&log.txt
    res=$?
    set +x
  else
    set -x
    peer chaincode instantiate -o orderer.tfc.com:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n ${CCNAME} -l ${LANGUAGE} -v ${VERSION} -c "${INSTARGS}" -P "${PERMISSION}" >&log.txt
    res=$?
    set +x
  fi
  cat log.txt
...
upgradeChaincode() {
  ...
  set -x
  peer chaincode upgrade -o orderer.tfc.com:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n $CCNAME -v 2.0 -c '{"Args":["init","a","90","b","210"]}' -P "AND ('Player1.peer','Player2.peer')"
  res=$?
  set +x
  cat log.txt
...
fetchChannelConfig() {
  ...
  if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
    set -x
    peer channel fetch config config_block.pb -o orderer.tfc.com:7050 -c $CHANNEL --cafile $ORDERER_CA
    set +x
  else
    set -x
    peer channel fetch config config_block.pb -o orderer.tfc.com:7050 -c $CHANNEL --tls --cafile $ORDERER_CA
    set +x
  fi
...
parsePeerConnectionParameters() {
  ...
  while [ "$#" -gt 0 ]; do
    ...
    PEERS="$PEERS $PEER"
    PEER_CONN_PARMS="$PEER_CONN_PARMS --peerAddresses $PEER.tfc.com:7051"
...
chaincodeInvoke() {
  ...
  if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
    set -x
    peer chaincode invoke -o orderer.tfc.com:7050 -C $CHANNEL_NAME -n $CCNAME $PEER_CONN_PARMS -c $INVOKARGS >&log.txt
    res=$?
    set +x
  else
    set -x
    peer chaincode invoke -o orderer.tfc.com:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n $CCNAME $PEER_CONN_PARMS -c $INVOKARGS >&log.txt
    res=$?
    set +x
  fi
  cat log.txt
